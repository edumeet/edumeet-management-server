...
export const tenantAdmin = (app: Application) => {
	...
	app.use('tenantAdmins', new TenantAdminService(getOptions(app)), {
	...
	// Initialize hooks
	app.service('tenantAdmins').hooks({
		around: {
			all: [
				authenticate('jwt'),
		...
		before: {
			all: [
				...
				schemaHooks.resolveQuery(tenantAdminQueryResolver)
			],
			find: [],
			get: [],
			create: [
...
declare module '../../declarations' {
	interface ServiceTypes {
		tenantAdmins: TenantAdminService
	}
}
