...
export const tenantUserManagedRoomNumberLimit = async (context: HookContext): Promise<void> => {
	...
	if (limit && rooms) {
		limit.forEach((element) => {
			if (element && element.userManagedRoomNumberLimit)
				if (element.userManagedRoomNumberLimit < rooms.total)
