...
export const tenantManagerManagedRoomNumberLimit = async (context: HookContext): Promise<void> => {
	...
	const rooms = await roomsService.find({
		query: {
			$limit: 0,
			creatorId: parseInt(context.params.user.id),
		}
	});

	if (limit && rooms) {
		limit.forEach((element) => {
			if (element && element.managerManagedRoomNumberLimit)
				if (element.managerManagedRoomNumberLimit < rooms.total)
					throw new Error('Room limit reached! (for this user)');	
		});
